...
class preConditionForTest(object):
    ...
    def __init__(self):
        ...
        self.local_mainDb='dianda_test_nj'
        """
        cityId:常州城市码
        """
        self.cityId=320100
    ...
    def newProduct(self):
        ...
        if product_id:
            ...
            areaPriceStr:价格定位
            """
            publish_data = {'isDirectSell': '0', 'combos[0][isFree]': '0', 'comboType': '0', 'catalogId': '2070000','vip': '0', 'yjPrice': '0', 'id': good_id,
                            'isHotFirst': '0', 'warehouseId': '13', 'title': publish_Name,'combos[0][id]': good_id, 'type': '0','combos[0][originalPrice]': '0',
                            'onSale': '0', 'dadou': '0', 'price': self.product_price,'isAllFirst': '0', 'combos[0][preWarehouseId]': '13',
                            'startTime': publish_time[0], 'isOrderLimit': 'unlimited', 'isTypeFirst': '0','endTime': publish_time[1], 'combos[0][packageNum]': '1',
                            'areaPriceStr': '156:{0}#157:{0}#166:{0}#'.format(self.product_price,self.product_price, self.product_price),
                            'notSoldPriceArea': '[]', 'combos[0][price]': '0', 'amount': '1', 'limit': self.procuct_limit,'isDiscount': '1'}
    ...
    def createProductTable(self):
        ...
        if CZ_data:
            cur = self.conn_local.cursor()
            self.conn_local.select_db(self.local_mainDb)
            cur.execute('create table if not exists productnj(id int ,name varchar(50),price VARCHAR(10),`limit` int(10),GoodId int(10),CityId int(10), PRIMARY KEY(id))')
            cur.execute('delete from productnj')
            cur.execute("insert into productnj(id,name,price,`limit`,GoodId,CityId)values('%d','%s','%s','%d','%d','%d')" % (CZ_data[0], CZ_data[1], CZ_data[2], CZ_data[3], CZ_data[4], CZ_data[5]))
