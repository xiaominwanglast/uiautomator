...
def chrome(options,chexe):
    pathsrc='C:\\chromeexe\\chromedriver.exe'
    num=0
    while True:
        try:
            ...
            print e
            num=num+1
            shutil.copyfile(chexe,pathsrc)
            time.sleep(3)
            if num>1:
                while(True):
                    chromeexe=raw_input(u"驱动崩溃，自动更新chrome失败，请手工更换驱动点击回车键Enter继续测试：".encode('gb18030'))
                    if chromeexe=='':
                        break

def run(path,listurl,chexe):
    ...
    for m in sheetalldata:
        ...
        for j in range(1,onenrows):
            ...
            driver=chrome(options,chexe)
            urlall=urldata(page,listurl)
            url=urlall[0]
            if type(data.sheet_by_name(page).cell_value(j,1))==float:
                key=str(int(data.sheet_by_name(page).cell_value(j,1)))
            else:
                key=data.sheet_by_name(page).cell_value(j,1)
            print url+key
            try:
...
chexe=file+'\\'+u'chromedriver.exe'
print u"web自动化打包版本V20170228.7".encode('gb18030')
print "-----------------------------------------------------------"
...
print u"***确保备用驱动e:\\mac\\id\\chromedriver.exe存在".encode('gb18030')
print "-----------------------------------------------------------"
time.sleep(2)
print "-----------------------------------------------------------"
...
time.sleep(1)
print "-----------------------------------------------------------"
print u"！！！由于chrome驱动与浏览器不兼容问题，造成程序不能持续调用驱动打开浏览器".encode('gb18030')
print u"1.新增驱动出现问题后程序挂起不退出，自动更换驱动，成功更换后自动继续web测试".encode('gb18030')
print u"2.新增驱动出现异常后，若更换成功后调用驱动异常程序挂起，等待人工参与检查".encode('gb18030')
print "-----------------------------------------------------------"
time.sleep(2)
