...
def getpagedata(page,driver):
    if page==u"首页":
        ...
        try:
            ...
            try:
                warmnewsup=driver.find_element_by_xpath("/html/body/div[5]/div/div[2]/div[3]/div[2]/script[2]").get_attribute("innerHTML").strip()
            except:
                warmnewsup=0
        try:
    ...
    if page==u"详情页":
        ...
        try:
            ...
            newsfocus=0
        try:
            guesslike=driver.find_element_by_xpath("/html/body/div[4]/div/div[3]/div[5]/div[2]").get_attribute('innerHTML').strip()
        except:
            guesslike=0
        try:
                            ...
                            if pagedata[m-2]!=0:
                                ...
                                try:
                                    if idvalue not in pagedata[m-2]:
                                        ...
                                        wb.get_sheet(0).write(rows,i,u"%s-qid(%s)存在异常，需复测"%(sheetpage.cell_value(0,m),key))
                                except:
                                    print  u"表(%s,%s)数据存在异常，需复测"%(rows,i)
                            else:
                                i=i+1
                                wb.get_sheet(0).write(rows,0,page)
                                wb.get_sheet(0).write(rows,1,key)
                                wb.get_sheet(0).write(rows,i,u"%s-qid(%s)抓取异常，需复测"%(sheetpage.cell_value(0,m),key))
                    break
                else:
                    wb.get_sheet(0).write(rows,0,page)
                    wb.get_sheet(0).write(rows,1,key)
                    wb.get_sheet(0).write(rows,2,u"-qid(%s)与表格qid(%s)不同,需核对"%(c['value'],key))
                    break
