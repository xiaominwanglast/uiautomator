...
class WXBot:
    ...
    def __init__(self):
       ...
       # self.db=self.cn.qutoutiao
       # self.table=self.db.result_other
        self.result_path11=os.path.dirname(os.getcwd())+'\\wx_1\\Test_Url_Result.xls'
   ...
   #     print dic
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if msg_type_id == 0:
         ...
         #   print content.encode('gb18030')
            ...
            uid = uid[:-1]
        #    print self.get_contact_name(uid)
            name = self.get_contact_prefer_name(self.get_contact_name(uid))
         #   print name
            if not name:
             #   print msg['FromUserName']
                print self.get_group_member_name(msg['FromUserName'], uid)
        ...
        if mtype == 1:
            if content.find('http://weixin.qq.com/cgi-bin/redirectforward?args=') != -1:
                ...
                if self.DEBUG:
                    try:
                     ...
                     #   self.save_pydata_other(msg_content['data'])
            ...
            if self.DEBUG:
                ...
                print '    --------------------------'
                self.write_sharedata(msg['FileName'],msg['Url'])
        elif mtype == 62:
            msg_content['type'] = 8
            msg_content['data'] = content
            if self.DEBUG:
        ...
        return msg_content
    def write_sharedata(self,title,url):
        wp=xlrd.open_workbook(self.result_path11)
        sheet=wp.sheet_by_name('Sheet1')
        cp=copy(wp)
        cp.get_sheet(0).write(sheet.nrows-1,1,title)
        cp.get_sheet(0).write(sheet.nrows-1,2,url)
        cp.save(self.result_path11)
        print '========Test_url:'+url
        time.sleep(4)
    def save_pydata(self,msg):
        if re.findall('^http://.*?',msg):
            try:
                mainurl=msg.split('/')[2]
                second_domain=mainurl.split('.')[0]
                first_domain='.'.join(mainurl.split('.')[1:])
                dic_={'url':msg,'mainurl':mainurl,'first_domain':first_domain,'second_domain':second_domain}
              #  self.table.save(dic_)
            except:
                print '----------------'*3
                print 'Test_error_url:'+msg
                print '----------------'*3

    def save_pydata_other(self,msg):
        if re.findall('^http://.*?',msg):
            try:
                mainurl=msg.split('/')[2]
                second_domain=mainurl.split('.')[0]
    ...
    def run(self):
        try:
            self.get_uuid()  #获取微信需要的uuid
            self.gen_qr_code(os.path.join(self.temp_pwd,'wxqr.png')) #把UUID附上url展示图片
            print '[INFO] Please use WeChat to scan the QR code .'

            result = self.wait4login()  #登陆成功返回200
            if result != SUCCESS:
                print '[ERROR] Web WeChat login failed. failed code=%s' % (result,)
                self.status = 'loginout'
                return

            if self.login():  #登陆成功后初始化一些登陆信息
            ...
            if self.init():  #获取所有联系人信息  获取自己账号self.my_account
                ...
                return
            self.status_notify()
            if self.get_contact():  #批量获取所有人员信息
                print '[INFO] Get %d contacts' % len(self.contact_list)
                print '[INFO] Start to process messages .'
            self.proc_msg()  #主要运行函数，接入任务接口
            self.status = 'loginout'
        except Exception,e:
