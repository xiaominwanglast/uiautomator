        ...
        for i in fa:
         ...
         #   print score[0][0],score[0][1],score[1][0],score[1][1]
            if score[0][0]!='':
                ...
                dict_['iphone']=score[0][0].split('*')[-1].replace(')','').strip()
                dict_['result']='pass'
                dict_['score']=score[1][0]
            else:
                dict_['name']=score[0][1].split('(')[0].strip()
                dict_['iphone']=score[0][1].split('*')[-1].replace(')','').strip()
                dict_['result']='fail'
                dict_['score']=score[1][1]
            list_all.append(dict_)
        return list_all
def do_data():
    listname=[u"仲　锦",u"张洒洒",{'user':u"李　鑫",'iphone':'5621'},u"徐培杰",u"侯新妮",u"刘　伟",u"田振华",u"邓琳仪",u"谭宝林",u"高慧敏",u'徐梦怀',u'刘　洋',u"程　远",u"谭佳佳",u"范石磊",u"何　刚",u'陈　珊',u"岳观文",u"王孝敏",u"周志勇",u"许　红"]
    list_data=get()
    str_=''
    try:
        for name in listname:
            for i in list_data:
                if type(name)==dict:
                    if i['name']==name['user'] and i['iphone']==name['iphone']:
                        str_=str_+'\t'+name['user']+'\t'+i['score']+'\t'+i['result']+'\n'
                        break
                else:
                    if i['name']==name:
