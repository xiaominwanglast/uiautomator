...
def writereport(filepath,file,filereport,m,day):
    ...
    for name in listname:
        ...
        for i in range(dstrows):
            if sheet.cell_value(i,0)==name:
                ...
                _write_center(worksheet, "%s22"%listchar[M-1], float(day*8)-sheet.cell_value(i,14), workbook)
    #第一列
    ...
    for name in listname:
       ...
       # _write_center(worksheet, "%s22"%listchar[N-1], float(day*8), workbook)
...
print (u'请确保嵩恒产品研发管理平台bug表导出后另存.xlsx表（确保有Creator与Created两个字段）'.encode('gb18030'))
sleep(1)
filepath=raw_input(u'请拖入测试组工作日报与嵩恒产品研发管理平台bug表的文件夹位置(同一目录，确保有测试组工作日报与产品研发两个字段)：\n'.encode('gb18030'))
sleep(1)
