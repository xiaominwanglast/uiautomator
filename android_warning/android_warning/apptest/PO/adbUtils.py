...
class ADB(object):
    ...
    def getAppVersion(self, keyword):
        """
        模糊查询与keyword匹配的应用包名列表
        usage: getMatchingAppList("qq")
        out = self.shell("dumpsys window w | %s \/ | %s name=" % (find_util, find_util)).stdout.read()
 ...
 #       out=self.shell("dumpsys package %s| %s %s %(keyword,find_util,ver)").stdout.read()
 #       out=self.shell("dumpsys package %s| %s %s"%(keyword,find_util,ver)).stdout.read().strip()
        for l in self.shell("dumpsys package %s| %s %s"%(keyword,find_util,ver)).stdout.readlines():
            out.append(l.split("=")[-1].splitlines()[0])
